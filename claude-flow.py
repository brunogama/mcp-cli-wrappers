...
# ///
"""
CLI Wrapper for Claude Flow MCP
Auto-generated wrapper with 4-level progressive disclosure.

Run with: uv run claude-flow.py [COMMAND] [ARGS]
...
Use:
  ...
  uv run claude-flow.py info FUNCTION_NAME
  uv run claude-flow.py example FUNCTION_NAME
  uv run claude-flow.py FUNCTION_NAME [ARGS]
...
FUNCTION_EXAMPLES = {
    ...
    "task_orchestrate": [
        {
            "title": "Example",
            "command": "uv run WRAPPER.py task_orchestrate"
        }
    ]
}
...
def main():
    parser = argparse.ArgumentParser(
        prog="claude-flow",
        description="Claude Flow MCP Wrapper",
...
EXAMPLES:
  ...
  uv run claude-flow.py list
  uv run claude-flow.py info my_function
  uv run claude-flow.py example my_function
  uv run claude-flow.py my_function --arg value
